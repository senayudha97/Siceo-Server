...
class Setup_zv_generator extends Guide
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get('setup_zv_generator')->result_array();


        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ampenan/setup_zv_generator_view', $data);
    ...
    public function add()
    {
        ...
        $this->Tbl_ampenan->insert_setup_zv_generator($arr_data);
        $this->flash_success('Menambah setup zav generator');
        redirect('setup_zv_generator');
    }

    public function edit($param = NULL)
    {
        if ($param == 1) {
            $id = $_GET['pk_setup_zv_generator'];

            $this->db->where('pk_setup_zv_generator', $id);
            $result = $this->db->get('setup_zv_generator')->row_array();
            ...
            $this->Tbl_ampenan->update_setup_zv_generator($data);
    ...
    {
        $this->db->where('pk_setup_zv_generator', $param);
        $this->db->delete('setup_zv_generator');
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("setup_zv_generator", ['pk_setup_zv_generator' => $id])->row_array();
