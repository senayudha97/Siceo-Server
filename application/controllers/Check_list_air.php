...
class Check_list_air extends Guide
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get_where('checklist_air', ['is_delete' => 0])->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('plts/check_list_air_view', $data);
    ...
    public function edit($param = NULL)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('plts/check_list_air_edit', $data);
            $this->load->view('templates/footer');
        } else {
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_plts->delete_air($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM checklist_air WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/checklist_air.php');
            ...
            $data = $this->load->view('plts/check_list_air_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
