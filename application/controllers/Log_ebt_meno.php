...
class Log_ebt_meno extends Guide //ini adalah  Log PLMTH meno
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get_where('log_ebt_meno', ['is_delete' => 0])->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ebt/Log_ebt_meno_view', $data);
    ...
    public function edit($param = NULL)
    {
        ...
        $data['edit'] = $this->db->get('log_ebt_meno')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ebt/Log_ebt_meno_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ebt->update_ebt_meno($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_ebt->delete_ebt_meno($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_ebt_meno", ['pk_log_tengah_jaman' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_ebt_meno WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/log_ebt_meno_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "log_ebt_meno__rep " . date('d-m-Y') . '.xlsx';
            ...
            $data = $this->load->view('ebt/Log_ebt_meno_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
