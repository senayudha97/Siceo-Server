...
class Log_paok_motong_checklist extends Guide
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('log_paok_motong_checklist');
        $this->db->join('user', 'user.id = log_paok_motong_checklist.insert_by');
        $data['data'] = $this->db->get('')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('paok_motong/checklist_view', $data);
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_checklist', $param);
        $data['edit'] = $this->db->get('log_paok_motong_checklist')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('paok_motong/checklist_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_paok_motong->update_checklist($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_paok_motong->delete_checklist($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_paok_motong_checklist", ['pk_checklist' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_paok_motong_checklist WHERE tanggal_input = '$awal' AND log_paok_motong_checklist.is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/checklist_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "checklist_Rep " . date('d-m-Y') . '.xlsx';
            ...
            $data = $this->load->view('paok_motong/checklist_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
