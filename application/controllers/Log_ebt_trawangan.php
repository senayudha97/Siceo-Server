...
class Log_ebt_trawangan extends Guide //ini adalah  Log PLMTH trawangan
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model("Tbl_ebt");
        is_logged_in();
    }
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get_where('log_ebt_trawangan', ['is_delete' => 0])->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ebt/Log_ebt_trawangan_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ebt/Log_ebt_trawangan_add', $data);
            $this->load->view('templates/footer');
        } else {
            $arr_data = $this->input->post('input');
            $arr_data['tanggal_input'] = date_to_db($arr_data['tanggal_input']);

            $this->Tbl_ebt->insert_ebt_trawangan($arr_data);
            $this->flash_success('Menambah Log Tengah Jaman');
            redirect('Log_ebt_trawangan');
        }
    }
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_log_tengah_jaman', $param);
        $data['edit'] = $this->db->get('log_ebt_trawangan')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ebt/Log_ebt_trawangan_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ebt->update_ebt_trawangan($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_ebt->delete_ebt_trawangan($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_ebt_trawangan", ['pk_log_tengah_jaman' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_ebt_trawangan WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/log_ebt_trawangan_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "log_ebt_trawangan__rep " . date('d-m-Y') . '.xlsx';
            ...
            // Printout
            $filename = 'Log Narmada Tengah Jaman' . date('d-m-Y');
            $data = $this->load->view('ebt/Log_ebt_trawangan_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
