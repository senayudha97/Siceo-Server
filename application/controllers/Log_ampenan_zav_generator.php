...
class Log_ampenan_zav_generator extends Guide
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('log_zav_generator');
        $this->db->join('user', 'user.id = log_zav_generator.insert_by');
        $data['data'] = $this->db->get('')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ampenan/log_zav_generator_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_zav_generator_add', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ampenan->insert_zav_generator($arr_data);
            $this->Tbl_ampenan->auto_insert_multiple($arr_multi, 'log_zav_generator_detail');
            $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible">
            <button type="button" class="close" data-dismiss="alert">&times;</button>
            <strong>Success!</strong> Menambah laporan zav generator
            </div>');
            redirect('Log_ampenan_zav_generator');
        }
    }
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_zav_generator', $param);
        $data['edit'] = $this->db->get('log_zav_generator')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_zav_generator_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ampenan->update_zav_generator($data);
    ...
    {
        $user = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $this->Tbl_ampenan->delete_zav_generator($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    {
        $data = [
            'title' => 'Log Zav Generator Detail',
            'user' => $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array()
        ];
        ...
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ampenan/log_zav_generator_detail', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_zav_generator", ['pk_zav_generator' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_zav_generator WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/zav_generator_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "Zav_Generator_rep " . date('d-m-Y') . '.xlsx';
            ...
            // Printout
            $filename = 'Log Zav Generator' . date('d-m-Y');
            $data = $this->load->view('ampenan/log_zav_generator_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
