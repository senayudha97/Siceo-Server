...
class Log_paok_motong_beban_penyulang extends Guide
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('log_beban_penyulang');
        $this->db->join('user', 'user.id = log_beban_penyulang.insert_by');
        $data['data'] = $this->db->get('')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('paok_motong/log_beban_penyulang_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('paok_motong/log_beban_penyulang_add', $data);
            $this->load->view('templates/footer');
        } else {
            $arr_data = $this->input->post('input');
            $arr_data['tanggal_input'] = date_to_db($arr_data['tanggal_input']);

            $this->Tbl_paok_motong->insert_beban_penyulang($arr_data);
            $this->flash_success('Menambah beban penyulang');
            redirect('Log_paok_motong_beban_penyulang');
        }
    }

    public function edit($param = NULL)
    {
        $data['title'] = 'Log Beban Penyulang';
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();

        $this->db->where('pk_beban_penyulang', $param);
        $data['edit'] = $this->db->get('log_beban_penyulang')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('paok_motong/log_beban_penyulang_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_paok_motong->update_beban_penyulang($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_paok_motong->delete_beban_penyulang($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_beban_penyulang", ['pk_beban_penyulang' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_beban_penyulang WHERE tanggal_input = '$awal' AND log_beban_penyulang.is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/beban_penyulang_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "Beban_Penyulang_Rep " . date('d-m-Y') . '.xlsx';
            ...
            $data = $this->load->view('paok_motong/log_beban_penyulang_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
