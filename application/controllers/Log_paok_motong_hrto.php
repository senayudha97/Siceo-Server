...
class Log_paok_motong_hrto extends Guide
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('log_hrto');
        $this->db->join('user', 'user.id = log_hrto.insert_by');
        $data['data'] = $this->db->get('')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('paok_motong/log_hrto_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('paok_motong/log_hrto_add', $data);
            $this->load->view('templates/footer');
        } else {
            $arr_data = $this->input->post('input');
            $arr_data['tanggal_input'] = date_to_db($arr_data['tanggal_input']);
            $this->Tbl_paok_motong->notif_hrto($arr_data, $data['user']);
            $this->Tbl_paok_motong->insert_hrto($arr_data);
            $this->flash_success('Menambah HRTO');
            redirect('Log_paok_motong_hrto');
        }
    }
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_hrto', $param);
        $data['edit'] = $this->db->get('log_hrto')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('paok_motong/log_hrto_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_paok_motong->update_hrto($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_paok_motong->delete_hrto($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_hrto", ['pk_hrto' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_hrto WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/hrto_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "HRTO_rep " . date('d-m-Y') . '.xlsx';
            ...
            // Printout
            $filename = 'Log HRTO' . date('d-m-Y');
            $data = $this->load->view('paok_motong/log_hrto_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
