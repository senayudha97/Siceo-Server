...
class Log_ampenan_penyulang_taman extends Guide
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get('log_penyulang_taman')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ampenan/log_penyulang_taman_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_penyulang_taman_add', $data);
            $this->load->view('templates/footer');
        } else {
            $arr_data = $this->input->post('input');
            $arr_data['tanggal_input'] = date_to_db($arr_data['tanggal_input']);

            $this->Tbl_ampenan->insert_penyulang_taman($arr_data);
            $this->flash_success('Menambah Penyulang Taman');
            redirect('log_ampenan_penyulang_taman');
        }
    }
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_penyulang_taman', $param);
        $data['edit'] = $this->db->get('log_penyulang_taman')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_penyulang_taman_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ampenan->update_penyulang_taman($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_ampenan->delete_penyulang_taman($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_penyulang_taman", ['pk_penyulang_taman' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_penyulang_taman WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/penyulang_taman_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "penyulang_taman_rep " . date('d-m-Y') . '.xlsx';
            ...
            // Printout
            $filename = 'Log Penyulang Taman' . date('d-m-Y');
            $data = $this->load->view('ampenan/log_penyulang_taman_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
