...
class Check_list_meno extends Guide
{
    ...
    public function index()
    {
        ...
        $data['data'] = $this->db->get_where('checklist_meno', ['is_delete' => 0])->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('plts/check_list_meno_view', $data);
    ...
    public function edit($param = NULL)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('plts/check_list_meno_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_plts->update_meno($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_plts->delete_meno($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM checklist_meno WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/checklist_meno.php');
            ...
            $data = $this->load->view('plts/check_list_meno_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
