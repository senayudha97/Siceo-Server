...
class Log_ampenan_zav_engine extends Guide
{
    ...
    public function index()
    {
        ...
        $this->db->select('*');
        $this->db->from('log_zav_engine');
        $this->db->join('user', 'user.id = log_zav_engine.insert_by');
        $data['data'] = $this->db->get('')->result_array();

        $this->load->view('templates/header', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('ampenan/log_zav_engine_view', $data);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_zav_engine_add', $data);
            $this->load->view('templates/footer');
        } else {
            $arr_data = $this->input->post('input');
            $arr_data['tanggal_input'] = date_to_db($arr_data['tanggal_input']);

            $this->Tbl_ampenan->insert_zav_engine($arr_data);
            $this->flash_success('Menambah Zav Engine');
            redirect('Log_ampenan_zav_engine');
        }
    }
    ...
    public function edit($param = NULL)
    {
        ...
        $this->db->where('pk_zav_engine', $param);
        $data['edit'] = $this->db->get('log_zav_engine')->row_array();
        $this->form_validation->set_rules('input[tanggal_input]', 'Tanggal Input', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('ampenan/log_zav_engine_edit', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $this->Tbl_ampenan->update_zav_engine($data);
    ...
    public function delete($param)
    {
        ...
        $this->Tbl_ampenan->delete_zav_engine($param, $user['id']);
        if ($this->db->affected_rows() == 1) {
    ...
    public function ajax()
    {
        ...
        $result = $this->db->get_where("log_zav_engine", ['pk_zav_engine' => $id])->row_array();
    ...
    public function report()
    {
        ...
        $param['log'] = $this->db->query("SELECT * FROM log_zav_engine WHERE tanggal_input = '$awal' AND is_delete = $this->is_delete ORDER BY tanggal_input ASC")->result_array();
        $param['awal']  = $awal;
        // $param['akhir'] = $akhir;
        ...
        if (isset($_POST['excel'])) {
            ...
            // Load Excel 
            require(APPPATH . 'excel/zav_engine_excel.php');

            // Execute to download the file
            $xl->getActiveSheet()->setTitle("Laporan Excel");

            $filename = "Zav_engine_rep " . date('d-m-Y') . '.xlsx';
            ...
            // Printout
            $filename = 'Log Zav Engine' . date('d-m-Y');
            $data = $this->load->view('ampenan/log_zav_engine_rep', $param, TRUE);
            $mpdf->WriteHTML($data);
